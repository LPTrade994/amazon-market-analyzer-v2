...
if use_fast:
    ...
    amazon_pct = pd.to_numeric(base.get("Buy Box: % Amazon 90 days").apply(parse_pct) if isinstance(base.get("Buy Box: % Amazon 90 days").iloc[0] if len(base)>0 else None, str) else base.get("Buy Box: % Amazon 90 days"), errors='coerce')
...
if use_oos:
    amazon_pct = pd.to_numeric(base.get("Buy Box: % Amazon 90 days"), errors='coerce')
    if amazon_pct.max() is not np.nan and amazon_pct.max() > 1.0:
